...
const { serializeMaterialRequest } = require('../utils/serialize');
  ...
  async (req, res, next) => {
    try {
      ...
      for (const item of req.body.items) {
        ...
        const line = mr.items.find((li) => li.materialId.toString() === item.materialId);
      ...
      res.status(201).json({
        data: {
          ...
          indent: serializeMaterialRequest(populated),
        },
      });
    } catch (err) {
      next(err);
